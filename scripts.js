...
(function() {
  ...
  function injectLogos(cfg) {
    ...
    // Desktop center logo (non-home pages)
    const navLogo = document.querySelector('.nav-logo-center');
    if (navLogo) navLogo.src = logoUrl;
    // Mobile logo
    const mobileLogo = document.querySelector('.nav-mobile-logo-link img');
    if (mobileLogo) mobileLogo.src = logoUrl;
    // Footer logo (rendered later by renderFooter)
  }
  ...
  function renderCategory(cfg, slug) {
    ...
    // Mobile-only marquee version: duplicate items so the scroll loops seamlessly
    const heroMarquee = document.querySelector('.cat-hero-marquee-track');
    if (heroMarquee && cat.heroMeta) {
      const items = cat.heroMeta.map(m => `<span class="cat-hero-marquee-item">${safe(m)}</span>`).join('');
      // Duplicate 4x for smooth infinite scroll
      heroMarquee.innerHTML = items + items + items + items;
    }

    const introP = document.querySelector('.cat-intro p');
    if (introP) introP.innerHTML = cat.intro || '';
    // Muted photo behind quote
    const introBg = document.querySelector('.cat-intro-bg');
    if (introBg && cat.quoteBg) introBg.style.backgroundImage = `url('${safe(cat.quoteBg)}')`;

    const grid = document.querySelector('.gallery-grid');
    if (grid && cat.gallery) {
      grid.innerHTML = '';
      cat.gallery.slice(0, 10).forEach((g, i) => {
        const item = el('div', { class: 'gallery-item reveal' });
        item.innerHTML = `
  ...
  function renderAbout(cfg) {
    ...
    // Muted photo behind story quote
    const storyBg = document.querySelector('.about-story-bg');
    if (storyBg && a.quoteBg) storyBg.style.backgroundImage = `url('${safe(a.quoteBg)}')`;

    const travelGrid = document.querySelector('.about-travel-grid');
    if (travelGrid && a.travelImages) {
      travelGrid.innerHTML = '';
      a.travelImages.slice(0, 3).forEach(img => {
