/* ==========================================================================
   Caitlin Onie Photography — Scripts (v2)
   ========================================================================== */
...
(function() {
  ...
  function initNav() {
    ...
    const links = nav.querySelector('.nav-links') ||
                  document.createElement('div'); // for non-home pages, links are split

    if (toggle) {
      toggle.addEventListener('click', () => {
        // Find the menu container — could be .nav-links (home) or .nav-links-left (others)
        const allLinks = nav.querySelectorAll('.nav-links, .nav-links-left, .nav-links-right');
        const isOpen = !toggle.classList.contains('open');
        toggle.classList.toggle('open', isOpen);
        // Use the first .nav-links for the mobile drawer
  ...
  function injectLogos(cfg) {
    ...
    // Centered nav logo (non-home pages)
    const navLogo = document.querySelector('.nav-logo-center');
    if (navLogo) navLogo.src = cfg?.brand?.logoDark || logoUrl;
  ...
  function renderHome(cfg) {
    if (!cfg) return;
    const h = cfg.hero || {};
    const a = cfg.aboutHome || cfg.aboutTeaser || {};
    ...
    if (imgMain) imgMain.style.backgroundImage = `url('${safe(a.imageMain || a.image)}')`;
    const imgAccent = document.querySelector('.about-home-img-accent');
    if (imgAccent) imgAccent.style.backgroundImage = `url('${safe(a.imageAccent || a.image)}')`;
  ...
  // ---------- Category page ----------
  // Smart gallery sizing — cycles through patterns to fill all gaps in the grid
  function getGallerySize(idx, isFeatured) {
    if (isFeatured) return 'size-large';
    // 8-step pattern — designed to balance the 12-col grid with no gaps
    const pattern = [
      'size-tall',     // 4-col tall
      'size-wide',     // 6-col wide  ← row 1 fills 4+6=10... need padding
      'size-small',    // 4-col short ← but 4 fits since first row is dense
      'size-medium',   // 4-col med
      'size-medium',   // 4-col med
      'size-medium',   // 4-col med   ← 4+4+4 = 12 ✓
      'size-half-wide',// 8-col
      'size-square',   // 4-col       ← 8+4 = 12 ✓
    ];
