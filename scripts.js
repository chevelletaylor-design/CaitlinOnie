/* ==========================================================================
   Caitlin Onie Photography — Shared Scripts
   Handles: nav scroll state, mobile menu, hero carousel, scroll reveals,
   page-specific rendering from config.json
   ========================================================================== */
...
(function() {
  ...
  // ---------- Nav: scroll state, mobile toggle, dynamic logo ----------
  function initNav() {
    ...
    function onScroll() {
      ...
      // On home page, hide logo until past hero (so the giant centered hero logo doesn't compete)
      if (document.body.classList.contains('page-home')) {
        const heroH = window.innerHeight - 60;
        nav.classList.toggle('hide-logo', y < heroH * 0.6);
      }
    }
  ...
  function renderNavLogo(cfg) {
    const logo = document.querySelector('.nav-logo');
    if (logo) logo.textContent = cfg.brand?.shortName || 'Caitlin Onie';
  }
    ...
    f.innerHTML = `
      <div class="footer-grid">
        <div class="footer-brand">
          <div class="display">${safe(fb.shortName || 'Caitlin Onie')}</div>
        ...
        <div class="footer-col">
          <h4>Locations</h4>
          <ul>
            ${(c.locations || []).map(l => `<li>${safe(l)}</li>`).join('')}
          </ul>
        </div>
        <div class="footer-col">
          <h4>Contact</h4>
  ...
  function renderHome(cfg) {
    ...
    const a = cfg.aboutTeaser || {};

    // Hero text
    const eyeLeft = document.querySelector('.hero-eyebrow .est');
    if (eyeLeft) eyeLeft.textContent = h.estLabel || 'EST.';
    ...
    const meta = document.querySelector('.hero-bottom-meta');
    if (meta) {
      meta.innerHTML = `
        <span>${safe(h.bottomLeft || 'Photographer')}</span>
        <span class="hero-scroll-cue">${safe(h.bottomRight || 'Scroll')}</span>
      `;
    }
    ...
    // Intro quote
    const introEl = document.querySelector('.intro-quote');
    if (introEl && i.quote) introEl.innerHTML = i.quote;
    const byline = document.querySelector('.intro-byline');
    if (byline) byline.textContent = i.byline || '';
    ...
    // About teaser
    const aboutImg = document.querySelector('.about-teaser-img');
    if (aboutImg) aboutImg.style.backgroundImage = `url('${safe(a.image)}')`;
    const aboutTitle = document.querySelector('.about-teaser-title');
    if (aboutTitle) aboutTitle.innerHTML = a.title || '';
    const aboutBody = document.querySelector('.about-teaser-body');
    if (aboutBody) aboutBody.textContent = a.body || '';

    // Travel marquee
    const marquee = document.querySelector('.travel-marquee-track');
    if (marquee && cfg.travelMarquee) {
      const words = cfg.travelMarquee;
      const items = words.concat(words).map(w => `<span class="travel-marquee-item">${safe(w)}</span>`).join('');
  ...
  function renderCategory(cfg, slug) {
    ...
    const heroLabel = document.querySelector('.cat-hero-title-block .label');
    if (heroLabel) heroLabel.textContent = cat.eyebrow || 'Photography';
    ...
    if (heroMeta) heroMeta.innerHTML = (cat.heroMeta || []).map(safe).join('<br>');
    ...
    if (grid && cat.gallery) {
      grid.innerHTML = '';
      const sizes = ['size-m', 'size-s', 'size-l', 'size-s', 'size-m', 'size-w', 'size-s', 'size-m', 'size-l', 'size-s'];
      cat.gallery.forEach((g, i) => {
        const size = g.featured ? 'size-l' : sizes[i % sizes.length];
  ...
  function renderInquire(cfg) {
    ...
    // Pre-fill subject if package query param exists
  ...
  async function init() {
    ...
    if (cfg) {
      ...
      if (body.classList.contains('page-home')) renderHome(cfg);
        ...
        // Category page — figure out slug from class
    ...
    // After render, initialize reveals
